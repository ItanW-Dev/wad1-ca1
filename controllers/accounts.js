...
const accounts = {
  ...
  login(request, response) {
    const viewData = {
      title: 'Login to the Guitar Collection Service',
    };
  ...
  logout(request, response) {
    response.cookie('guitarcollection', '');
    response.redirect('/');
  },
 ...
 //signup function to render signup page
  signup(request, response) {
    const viewData = {
      title: 'Signup to the Guitar Collection Service',
    };
...
authenticate(request, response) {
  ...
  if (user && user.password === request.body.password) {
    response.cookie('guitarcollection', user.email);
    logger.info('logging in ' + user.email);
    response.redirect('/start');
  } else {
    logger.warn('invalid login attempt for ' + request.body.email);
 ...
 //utility function getCurrentUser to check who is currently logged in
  getCurrentUser (request) {
    const userEmail = request.cookies.guitarcollection;
